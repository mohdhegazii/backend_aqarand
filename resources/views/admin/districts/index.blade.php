...
@section('header', __('admin.districts'))

@section('content')
    <div class="mb-4 flex flex-col md:flex-row justify-between space-y-4 md:space-y-0">
        <a href="{{ route($adminRoutePrefix.'districts.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-fit">
            @lang('admin.create_new')
        </a>
        <form action="{{ route($adminRoutePrefix.'districts.index') }}" method="GET" class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 rtl:md:space-x-reverse">
    ...
    <div class="bg-white shadow-md rounded my-6 overflow-x-auto">
        <form action="{{ route($adminRoutePrefix.'districts.bulk') }}" method="POST" id="bulk-form">
            @csrf
            ...
            <table class="min-w-full table-auto">
                <thead>
                    <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                        ...
                        <th class="py-3 px-6 text-start">@lang('admin.city')</th>
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    @foreach($districts as $district)
                        <tr class="border-b border-gray-200 hover:bg-gray-100 {{ !$district->is_active ? 'bg-gray-50 text-gray-400' : '' }}">
                            <td class="py-3 px-6 text-start">
                                <input type="checkbox" name="ids[]" value="{{ $district->id }}" class="row-checkbox">
                            </td>
                            <td class="py-3 px-6 text-start">
                                 {{ $district->city->display_name }} ({{ $district->city->region->display_name }})
                            </td>
                            <td class="py-3 px-6 text-start">
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2 rtl:space-x-reverse">
                                    <a href="{{ route($adminRoutePrefix.'districts.edit', $district) }}" class="text-purple-600 hover:text-purple-900">
                                        @lang('admin.edit')
                                    </a>
                                    <form action="{{ route($adminRoutePrefix.'districts.destroy', $district) }}" method="POST" onsubmit="return confirm('@lang('admin.confirm_delete')')" class="inline">
                                        @csrf
                                        @method('DELETE')
