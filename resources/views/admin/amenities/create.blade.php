...
@section('header', __('admin.create') . ' ' . __('admin.amenity'))

@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <form action="{{ route($adminRoutePrefix.'amenities.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <select name="amenity_type" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        @foreach(['project', 'unit', 'both'] as $type)
                            <option value="{{ $type }}" {{ old('amenity_type') == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="icon_class" value="{{ old('icon_class') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="e.g. bi-wifi">
                ...
                <div class="flex items-center justify-end space-x-4 rtl:space-x-reverse">
                    <a href="{{ route($adminRoutePrefix.'amenities.index') }}" class="text-gray-600 hover:text-gray-900">
                        @lang('admin.cancel')
                    </a>
