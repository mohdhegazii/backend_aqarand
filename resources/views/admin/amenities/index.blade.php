...
@section('content')
    <div class="mb-4 flex flex-col md:flex-row justify-between md:items-center gap-4">
        <a href="{{ route($adminRoutePrefix.'amenities.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-fit">
            @lang('admin.create_new')
        </a>
        <form action="{{ route($adminRoutePrefix.'amenities.index') }}" method="GET" class="flex flex-col md:flex-row gap-2">
    ...
    <div class="bg-white shadow-md rounded my-6 overflow-x-auto">
        <form action="{{ route($adminRoutePrefix.'amenities.bulk') }}" method="POST" id="bulk-form">
            @csrf
            ...
            <table class="min-w-full table-auto">
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    @foreach($amenities as $amenity)
                        <tr class="border-b border-gray-200 hover:bg-gray-100 {{ !$amenity->is_active ? 'bg-gray-50 text-gray-400' : '' }}">
                            <td class="py-3 px-6 text-start">
                                <input type="checkbox" name="ids[]" value="{{ $amenity->id }}" class="row-checkbox">
                            </td>
                            <td class="py-3 px-6 text-start">{{ $amenity->id }}</td>
                            <td class="py-3 px-6 text-start">
                                @if($amenity->image_path)
                                    <img src="{{ asset('storage/' . $amenity->image_path) }}" class="h-8 w-8 object-cover rounded-full" alt="Icon">
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2 rtl:space-x-reverse">
                                    <a href="{{ route($adminRoutePrefix.'amenities.edit', $amenity) }}" class="text-purple-600 hover:text-purple-900">
                                        @lang('admin.edit')
                                    </a>
                                    <form action="{{ route($adminRoutePrefix.'amenities.destroy', $amenity) }}" method="POST" onsubmit="return confirm('@lang('admin.confirm_delete')')" class="inline">
                                        @csrf
                                        @method('DELETE')
