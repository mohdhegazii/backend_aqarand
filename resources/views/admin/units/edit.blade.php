...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 bg-white border-b border-gray-200">
        <form method="POST" action="{{ route($adminRoutePrefix.'units.update', $unit) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <x-lookup.select
                        ...
                        :selected-id="old('project_id', $unit->project_id)"
                        :selected-text="$unit->project ? ($unit->project->name_en ?? $unit->project->name_ar) : ''"
                    />
                </div>
                <div>
                    ...
                    <x-lookup.select
                        ...
                        url="/admin/lookups/property-models"
                        placeholder="{{ __('admin.select_model') }}"
                        :selected-id="old('property_model_id', $unit->property_model_id)"
                        :selected-text="$unit->propertyModel ? ($unit->propertyModel->name_en ?? $unit->propertyModel->name_ar) : ''"
                    />
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    <x-lookup.property-hierarchy-picker
                        :selected-category-id="old('category_id', $unit->unitType->propertyType->category_id ?? null)"
                        :selected-property-type-id="old('property_type_id', $unit->unitType->property_type_id ?? null)"
                        :selected-unit-type-id="old('unit_type_id', $unit->unit_type_id)"
