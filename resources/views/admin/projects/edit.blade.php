        ...
        <form method="POST" action="{{ route('admin.projects.update', $project) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="delivery_year" id="delivery_year" value="{{ old('delivery_year', $project->delivery_year) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" min="2000" max="2100">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="region_id" id="region_id" onchange="fetchCities(this.value)" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        <option value="">@lang('admin.select_option')</option>
                        {{-- Pre-populate handled by JS or simple loop if passed, but JS is cleaner for chained selects. For Edit, we can pre-render if we pass related models, or just rely on JS fetch on load if we want dynamic but that's async. For now, let's keep it simple: assume user re-selects if they change parent, or better: pre-load the selected options. --}}
                        {{-- Since we didn't pass all regions/cities to view, we rely on JS or we should pass them. Let's pass the current selection options at least or just let the user see the ID if we don't load them.
                             Better: Let's use the API to load them on mount. --}}
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_en" id="description_en" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description_en', $project->description_en) }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label for="description_ar" class="block text-sm font-medium text-gray-700">Description (AR)</label>
                    <textarea name="description_ar" id="description_ar" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description_ar', $project->description_ar) }}</textarea>
                </div>
                <div>
