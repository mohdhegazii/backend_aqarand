    ...
    <div class="p-6 bg-white border-b border-gray-200">
        <form method="POST" action="{{ route('admin.projects.update', $project) }}">
            ...
            <div class="mb-8 border-b pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="lat" id="lat" value="{{ old('lat', $project->lat) }}" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm">
                    </div>
                    <div>
                        <label for="lng" class="block text-sm font-medium text-gray-700">@lang('admin.lng')</label>
                        <input type="text" name="lng" id="lng" value="{{ old('lng', $project->lng) }}" class="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm">
                    </div>
                </div>
            ...
            <div class="mb-8 border-b pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Status (Auto-calculated)</label>
                        <input type="text" value="{{ ucfirst(str_replace('_', ' ', $project->status)) }}" disabled class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 text-gray-500 shadow-sm">
                        <!-- Keep current status unless updated by logic -->
                        <input type="hidden" name="status" value="{{ $project->status }}">
                    </div>
                     <div>
                        <label class="block text-sm font-medium text-gray-700">Delivery Year (Auto-calculated)</label>
                        <input type="text" value="{{ $project->delivery_year ?? '-' }}" disabled class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 text-gray-500 shadow-sm">
                    </div>
                </div>
            </div>
                ...
                <div x-show="tab === 'en'" class="py-4">
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="address_en" id="address_en" value="{{ old('address_en', $project->address_en) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        </div>
                        <div>
                            <label for="description_en" class="block text-sm font-medium text-gray-700">Description (EN)</label>
                            <textarea name="description_en" id="description_en" class="wysiwyg">{{ old('description_en', $project->description_en) }}</textarea>
                        </div>
                        ...
                        <div class="border-t pt-4 mt-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="seo_slug_en" value="{{ old('seo_slug_en', $project->seo_slug_en) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Meta Title (EN)</label>
                                    <input type="text" name="meta_title_en" value="{{ old('meta_title_en', $project->meta_title_en) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700">Meta Description (EN)</label>
                                    <textarea name="meta_description_en" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('meta_description_en', $project->meta_description_en) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="tab === 'ar'" class="py-4" style="display: none;">
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="address_ar" id="address_ar" value="{{ old('address_ar', $project->address_ar) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        </div>
                        <div>
                            <label for="description_ar" class="block text-sm font-medium text-gray-700">Description (AR)</label>
                            <textarea name="description_ar" id="description_ar" class="wysiwyg">{{ old('description_ar', $project->description_ar) }}</textarea>
                        </div>
                         ...
                         <div class="border-t pt-4 mt-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="seo_slug_ar" value="{{ old('seo_slug_ar', $project->seo_slug_ar) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Meta Title (AR)</label>
                                    <input type="text" name="meta_title_ar" value="{{ old('meta_title_ar', $project->meta_title_ar) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700">Meta Description (AR)</label>
                                    <textarea name="meta_description_ar" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('meta_description_ar', $project->meta_description_ar) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    // --- Map Logic ---
    let map, marker;
    const initialLat = {{ $project->lat ?? 30.0444 }};
    const initialLng = {{ $project->lng ?? 31.2357 }};

    document.addEventListener('DOMContentLoaded', function() {
        // Initialize Map
        map = L.map('map').setView([initialLat, initialLng], {{ $project->lat ? 14 : 6 }});
    ...
    // --- Search Logic (Same as Create) ---
    // (We could extract this to a JS file, but inline is fine for now)
