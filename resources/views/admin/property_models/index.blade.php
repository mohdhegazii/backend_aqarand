...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 bg-white border-b border-gray-200">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route($adminRoutePrefix.'property-models.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                @lang('admin.create_new')
            </a>
        </div>

        <div class="mb-4">
             <form method="GET" action="{{ route($adminRoutePrefix.'property-models.index') }}" class="flex gap-4">
                    ...
                    @foreach($propertyModels as $model)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route($adminRoutePrefix.'property-models.edit', $model) }}" class="text-indigo-600 hover:text-indigo-900 mx-2">@lang('admin.edit')</a>
                                <form action="{{ route($adminRoutePrefix.'property-models.destroy', $model) }}" method="POST" class="inline" onsubmit="return confirm('@lang('admin.confirm_delete')')">
                                    @csrf @method('DELETE')
