...
@section('content')
    ...
    <div class="mb-4 flex flex-col md:flex-row justify-between space-y-4 md:space-y-0">
        <a href="{{ route('admin.unit-types.create', ['locale' => $locale]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-fit">
            @lang('admin.create_new')
        </a>
        <form action="{{ route('admin.unit-types.index', ['locale' => $locale]) }}" method="GET" class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 rtl:md:space-x-reverse">
    ...
    <div class="bg-white shadow-md rounded my-6 overflow-x-auto">
        <form action="{{ route('admin.unit-types.bulk', ['locale' => $locale]) }}" method="POST" id="bulk-form">
            @csrf
                        ...
                        <tr class="border-b border-gray-200 hover:bg-gray-100 {{ !$type->is_active ? 'bg-gray-50 text-gray-400' : '' }}">
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2 rtl:space-x-reverse">
                                    <a href="{{ route('admin.unit-types.edit', ['locale' => $locale, 'unit_type' => $type]) }}" class="text-purple-600 hover:text-purple-900">
                                        @lang('admin.edit')
                                    </a>
                                    <form action="{{ route('admin.unit-types.destroy', ['locale' => $locale, 'unit_type' => $type]) }}" method="POST" onsubmit="return confirm('@lang('admin.confirm_delete')')" class="inline">
                                        @csrf
                                        @method('DELETE')
