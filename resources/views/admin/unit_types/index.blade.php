...
@section('header', __('admin.unit_types'))

@section('content')
    <div class="mb-4 flex flex-col md:flex-row justify-between space-y-4 md:space-y-0">
        <a href="{{ route($adminRoutePrefix.'unit-types.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-fit">
            @lang('admin.create_new')
        </a>
        <form action="{{ route($adminRoutePrefix.'unit-types.index') }}" method="GET" class="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 rtl:md:space-x-reverse">
    ...
    <div class="bg-white shadow-md rounded my-6 overflow-x-auto">
        <form action="{{ route($adminRoutePrefix.'unit-types.bulk') }}" method="POST" id="bulk-form">
            @csrf
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    @foreach($unitTypes as $type)
                        <tr class="border-b border-gray-200 hover:bg-gray-100 {{ !$type->is_active ? 'bg-gray-50 text-gray-400' : '' }}">
                            ...
                            <td class="py-3 px-6 text-start">
                                <div class="font-semibold">{{ $type->name_en }}</div>
                                <div class="text-xs text-gray-500">{{ $type->name_local }}</div>
                            </td>
                            <td class="py-3 px-6 text-start">
                                {{ $type->code }}
                            </td>
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2 rtl:space-x-reverse">
                                    <a href="{{ route($adminRoutePrefix.'unit-types.edit', ['unitType' => $type]) }}" class="text-purple-600 hover:text-purple-900">
                                        @lang('admin.edit')
                                    </a>
                                    <form action="{{ route($adminRoutePrefix.'unit-types.destroy', ['unitType' => $type]) }}" method="POST" onsubmit="return confirm('@lang('admin.confirm_delete')')" class="inline">
                                        @csrf
                                        @method('DELETE')
