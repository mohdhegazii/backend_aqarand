...
@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <form action="{{ route($adminRoutePrefix.'property-types.update', $propertyType) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="category_id" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $propertyType->category_id) == $category->id ? 'selected' : '' }}>{{ $category->display_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="group_key" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        @foreach($groupKeys as $key)
                            <option value="{{ $key }}" {{ old('group_key', $propertyType->group_key) == $key ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $key)) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-center justify-end space-x-4 rtl:space-x-reverse">
                    <a href="{{ route($adminRoutePrefix.'property-types.index') }}" class="text-gray-600 hover:text-gray-900">
                        @lang('admin.cancel')
                    </a>
